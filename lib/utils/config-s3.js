...
module.exports = Config.extend({
  ...
  /*
    Holds options set by methods in this file.
    Eventually this gets merged into and overwrites any options defined in config-s3.json
    @property _options
    @private
  */
  _options: {},
    ...
    return this.checkSharedCreds()
      ...
      .then(function(options) {
        if (self.config.verbose) {
          options.logger = self.ui.outputStream;
        }
        return options;
      });
  },
  ...
  */
  ...
  /*
    Prompts user whether or not to use the found AWS shared credentials file. This can be silenced
    from config-s3.json file using the option `promptCredFile`.
    If no (n) then user will be prompted to provide keys, unless they exist as values
    in `process.env`(shell env). The values looked for are `AWS_ACCESS_KEY_ID` and `AWS_SECRET_ACCESS_KEY`
  ...
  */
  ...
  /*
    Reads 'additionalOptions' from config-s3.json
    If an additional option is defined **and** was already found (e.g. listed in the same config-s3.json file or given as a CLI option),
    then the user will STILL be prompted. Only put things here that you want to be prompted for.
    But if you want to keep the definition in `additionalOptions` while avoiding the prompt, then just include it in the `options` hash.
    If `options[additionalOption[i].name]` is defined, (for all i) the prompt will be skipped.
  ...
  */
  ...
  /*
    TODO: prompt to save the config file.

    Merges `this._options` hash into `options` found in config-s3.json.
    Prompts to save the file if it's different.

    @method saveConfigFile
    @return Promise -> this.options
  */
  saveConfigFile: function() {
    var options = this.mergeOptions();
    return options;
  },
  ...
  /*
    ...
    i.e.
      ...
      - If you want an s3 option to be pulled from a CLI options, define it there (this is the only case where an option wil overwrite a prev. defined one).
      - If you want an s3 options to come from congig-s3.json, only define it there.
      - If you want to be prompted for an option, define it in the `additionalOptions` hash in config-s3.json
    ...
    **note** once an option is found, you normally won't be prompted to overwrite it.
    If you want to be prompted then you must:
      - Define a prompt in the `additionalOptions` hash **and** make sure it's not included in `options` hash
