...
function merge() {
  ...
  for (i = 1; i < length; i++) {
    ...
    for (key in copy) {
      ...
      // combine arrays
      } else if (obj[key] && Array.isArray(obj[key]) && Array.isArray(value) ) {
        obj[key] = obj[key].concat(value);

      // add to array
      } else if (obj[key] && Array.isArray(obj[key]) && 'string' === typeof value) {
        obj[key].push(value);

      // add to array without modifying copy's array
      } else if (obj[key] && 'string' === typeof obj[key] && Array.isArray(value) ) {
        arrayAsValue = value.slice()
        arrayAsValue.push(obj[key]);
        obj[key] = arrayAsValue;

      // simple override
      } else if (obj[key] && 'undefined' !== typeof value) {
        obj[key] = value;
      }
    }
  }
