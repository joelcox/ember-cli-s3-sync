...
module.exports = {
  ...
  init: function() {
    ...
    this.config = fileTool.requireSilent(configPath) || require('../../package.json')['config'] || {};
  },
  ...
  extraStep: function(when, options) {
    var ui = this.ui;
    var step = this.config.extraSteps && this.config.extraSteps[when] || {};
    ui.writeLine('Running command: ' + chalk.yellow(when));

    return runCommand(step.command, ui, options, step.includeOptions)
